...
class BagClassifier(nn.Module):
    ...
    Output: ``y_hat`` in R^K           (K = number of classes; 2 for binary models)
    """

    def __init__(self, embed_dim: int = 128, n_classes: int = 2) -> None:
        super().__init__()
    ...
    def forward(self, z_tube: torch.Tensor) -> torch.Tensor:
        """Args: z_tube ``(embed_dim,)``. Returns: y_hat ``(K,)`` logits."""
