...
def main():
    ...
    # Rank pairs by mean_W (mean Wilcoxon W across cell types).
    # max-W saturates at 55 for ~14% of pairs with 18 cell types and n=10 donors.
    # mean-W is a continuous score that reflects consistency across ALL cell types,
    # providing much better pair discrimination.
    # Aggregate by max mean_W across exps (higher = stronger consistent signal).
    # Falls back to max W_stat, then min p_bonf for older top_pairs.json format.
    thresholds = np.arange(0.01, 0.51, 0.01)

    pair_best_mean_W: dict = {}
    pair_max_W: dict = {}
    pair_min_p: dict = {}
    for exp_pairs in geo_pairs_per_exp:
        for entry in exp_pairs:
            ...
            mW  = float(entry.get("mean_W", 0.0))
            W   = float(entry.get("W_stat", 0.0))
            pair_min_p[key] = min(pair_min_p.get(key, 1.0), p)
            pair_max_W[key] = max(pair_max_W.get(key, 0.0), W)
            pair_best_mean_W[key] = max(pair_best_mean_W.get(key, 0.0), mW)

    use_mean_W = any(v > 0 for v in pair_best_mean_W.values())
    use_W = any(v > 0 for v in pair_max_W.values())
    if use_mean_W:
        ranked = sorted(pair_best_mean_W.items(), key=lambda x: -x[1])
        rank_label = "max mean_W (mean Wilcoxon W across cell types, higher = stronger)"
    elif use_W:
        ranked = sorted(pair_max_W.items(), key=lambda x: -x[1])
        rank_label = "max W_stat (fallback — mean_W not in top_pairs.json)"
    else:
        ranked = sorted(pair_min_p.items(), key=lambda x: x[1])
        rank_label = "min p_bonf (fallback — no W fields in top_pairs.json)"
    ...
    ax.set_xlabel("Top X% ordered pairs (by mean W across cell types)")
