"""
Bootstrap experiment — cluster job script.

Randomly samples N_SAMPLE_PER_GROUP simple and complex cytokines from
pre-defined hypothesis pools, then runs the full AB-MIL pipeline
(Stage 1 encoder + Stage 2 frozen MIL + Stage 3 fine-tuned MIL).

Results (figures + text summary + model checkpoints) are saved to:
    results/bootstrap_seed_{BOOTSTRAP_SEED}_{timestamp}/

Usage:
    python scripts/run_bootstrap.py --bootstrap_seed 42
    python scripts/run_bootstrap.py --bootstrap_seed 123 --n_sample 5
"""
...
# ---------------------------------------------------------------------------
# Helper — extra metrics (NormAUC, Pmax)
# ---------------------------------------------------------------------------
...
def _plot_learning_curves(dynamics, simple_cyts, complex_cyts, bootstrap_seed, out_dir):
    ...
    plt.tight_layout()
    fig.savefig(out_dir / f"learning_curves_bootstrap_{bootstrap_seed}.png",
...
def _plot_per_donor_curves(dynamics, subset_cyts, simple_cyts, complex_cyts,
    ...
    for cyt in subset_cyts:
        ...
        fig.savefig(
            out_dir / f"per_donor_curves_{safe_name}_bootstrap_{bootstrap_seed}.png",
...
def _plot_scatter(extra_train, extra_val, subset_cyts, simple_cyts, complex_cyts,
    ...
    plt.tight_layout()
    fig.savefig(out_dir / f"scatter_normauc_pmax_bootstrap_{bootstrap_seed}.png",
                dpi=150, bbox_inches="tight")
    plt.close(fig)
...
def main():
    parser = argparse.ArgumentParser(description="Bootstrap cytokine MIL experiment")
    ...
    # ------------------------------------------------------------------
    # Output directory — one per run, seed + timestamp to avoid collisions
    # ------------------------------------------------------------------
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S") + f"_pid{os.getpid()}"
    out_dir = Path(__file__).parent.parent / "results" / \
              f"bootstrap_seed{BOOTSTRAP_SEED}_{timestamp}"
    ...
    log(f"Bootstrap experiment — seed={BOOTSTRAP_SEED}  n_sample={N_SAMPLE_PER_GROUP}")
    ...
    # Tube datasets (preload=True for fast I/O during training)
    ...
    log()
    log("=" * 60)
    log("Stage 2 — MIL training (encoder frozen)")
    log("=" * 60)

    # LR override matching notebook
    stage2_lr       = 0.0002
    ...
    log("Dynamics Stage 2 records saved (dynamics_stage2.pkl).")

    # ------------------------------------------------------------------
    # 4. Stage 3 — Joint fine-tuning
    # ------------------------------------------------------------------
    log()
    log("=" * 60)
    log("Stage 3 — Joint fine-tuning (encoder unfrozen)")
    log("=" * 60)
    ...
    torch.save(mil_model.state_dict(),
               str(out_dir / f"mil_stage3_bootstrap_{BOOTSTRAP_SEED}.pt"))
    log("Stage 3 model saved.")

    # ------------------------------------------------------------------
    # 5. Dynamics analysis (Stage 2 as primary)
    # ------------------------------------------------------------------
    log()
    log("=" * 60)
    ...
    if set(train_order) == set(val_order):
        ...
        log()
        log(f"Train vs val rank correlation: Spearman rho = {rho:.3f}  (p={pval:.3f})")
        log("  rho > 0.7 → ranking generalizes to held-out donors.")
    ...
    log("Cytokine attention entropy summary")
    ...
    stability_s2_s3 = check_seed_stability([dynamics_stage2, dynamics_stage3], exclude=["PBS"])

    val_s2_order = [c for c, _ in val_learnability_result["ranking"]]
    val_s3_order = [c for c, _ in rank_cytokines_by_learnability(val_donor_traj_s3, exclude=["PBS"])["ranking"]]

    log("Stage 2 vs Stage 3 ranking correlation")
    log(f"  Train: Spearman rho = {stability_s2_s3['mean_rho']:.3f}")
    log(f"  Stable (rho > 0.7): {stability_s2_s3['stable']}")
    if set(val_s2_order) == set(val_s3_order):
        val_s3_aligned = [val_s3_order.index(c) for c in val_s2_order]
        rho_val_s3, _ = spearmanr(range(len(val_s2_order)), val_s3_aligned)
        log(f"  Val:   Spearman rho = {rho_val_s3:.3f}")
    ...
    log(f"  Saved: learning_curves_bootstrap_{BOOTSTRAP_SEED}.png")

    _plot_per_donor_curves(
        dynamics_stage2, SUBSET_CYTOKINES, SIMPLE_CYTOKINES, COMPLEX_CYTOKINES,
        BOOTSTRAP_SEED, out_dir)
    log(f"  Saved: per_donor_curves_<cytokine>_bootstrap_{BOOTSTRAP_SEED}.png (x{len(SUBSET_CYTOKINES)})")

    _plot_scatter(extra_train, extra_val, SUBSET_CYTOKINES,
                  SIMPLE_CYTOKINES, COMPLEX_CYTOKINES, BOOTSTRAP_SEED, out_dir)
    log(f"  Saved: scatter_normauc_pmax_bootstrap_{BOOTSTRAP_SEED}.png")
