"""Stage 2 of the §40 dropout+curation run — encode and persist the pseudo-tubes.
...
§40's encoder carries 50% dropout, but that changes nothing here: `nn.Dropout` is an
identity in `eval()` mode, the encoder is loaded in `eval()`, and the frozen encoder is
held there throughout Stage-2 training as well — so the cache is deterministic and the
cached and un-cached paths still agree exactly (cascadir's
`_assert_frozen_encoder_eval`).
