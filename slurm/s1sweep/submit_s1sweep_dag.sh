...
#   cluster_cmd "bash -l -c 'cd cytokine-mil && bash slurm/s1sweep/submit_s1sweep_dag.sh'"
#
# Dry run (prints the sbatch commands instead of submitting; synthetic job ids keep the
# --dependency chain readable):
#   SUBMIT=echo bash slurm/s1sweep/submit_s1sweep_dag.sh
#
...
#   * one afternotok sentinel per stage -> results/s1sweep/STATUS.md + FAILED_<stage>
#   * a self-resubmitting watchdog every 30 min -> results/s1sweep/HEALTH.md
#
# Bottom line: results/s1sweep/ARM_COMPARISON.md + arm_diversity.csv
set -e
...
for f in scripts/_encsweep_config.py scripts/prepare_s1sweep.py \
...
if [ -f "$OUT/DONE_analysis" ]; then
    ...
    echo "      Move results/s1sweep aside first if you want a clean re-run."
fi
...
sentinel () {
    submit --dependency=afternotok:"$2" --kill-on-invalid-dep=yes \
        ...
        slurm/s1sweep/sentinel.slurm
}

PREP=$(submit slurm/s1sweep/prepare.slurm)
S_PREP=$(sentinel prepare "$PREP")

ENC=$(submit --dependency=afterok:"$PREP" slurm/s1sweep/encoder.slurm)
S_ENC=$(sentinel encoder "$ENC")

TRAIN=$(submit --dependency=afterok:"$ENC" slurm/s1sweep/train.slurm)
S_TRAIN=$(sentinel train "$TRAIN")

IG=$(submit --dependency=afterok:"$TRAIN" slurm/s1sweep/ig.slurm)
S_IG=$(sentinel ig "$IG")

SIGN=$(submit --dependency=afterok:"$IG" slurm/s1sweep/sign.slurm)
S_SIGN=$(sentinel sign "$SIGN")

ANA=$(submit --dependency=afterok:"$IG",afterany:"$SIGN" slurm/s1sweep/analysis.slurm)
S_ANA=$(sentinel analysis "$ANA")

WATCH=$(submit slurm/s1sweep/watchdog.slurm)
...
echo ""
if [ "$DRYRUN" = "1" ]; then
  echo "Would submit the Stage-1 CONSTRUCTION sweep (synthetic ids above):"
else
  echo "Submitted the Stage-1 CONSTRUCTION sweep:"
fi
echo "  prepare   = $PREP    (CPU 64G:          2 replica arms + bank -> 2 volume arms)"
echo "  encoder   = $ENC    (GPU array 0-3:    pub_replica/…_clean/vol_small/vol_large)"
