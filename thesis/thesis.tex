...
\[
  S_X \;=\; \operatorname*{arg\,max}_{\substack{G\subseteq\mathcal{G}\\ |G|=k}}\ \operatorname{acc}\!\big(M^{X}_{G}\big),
\]
where $\mathcal{G}$ is the set of all genes, $\operatorname{acc}(M^{X}_{G})$ is the accuracy of a classifier that tells $X$ from the control using only the genes $G$, and $k$ fixes the signature's size.
...
The signature $S_X$ is defined by an objective, not given to us as data; lacking any ground truth for it, we recover it with an estimator. Because the model is nonlinear, the genes it relies on cannot be read off directly; Integrated Gradients \citep{sundararajan2017axiomatic} is a principled attribution method that recovers them by integrating the model's sensitivity from the baseline to the pseudo-tube. We keep the 50 genes it attributes most to, and these are our estimate of $S_X$. Crucially, these attributions are not interchangeable with differential expression: an ablation (Section~\ref{sec:results}) finds that a model-free fold-change list picks almost disjoint genes and loses the direction signal, confirming the model's attributions carry information a simple ranking does not.
...
\begin{tikzpicture}[line cap=round, scale=0.82]
  ...
  \node[above left, align=right]  at (inp)  {\scriptsize input\\(pseudo-tube)};
...
larger component (here gene~1) is kept for the signature $S_X$ and the smaller (gene~2) is
dropped; with a full panel of thousands of genes, the top-50 by attribution form $S_X$.}
\label{fig:ig3}
\end{figure}
...
\begin{tikzpicture}[line cap=round, scale=0.92]
  ...
  \node[green!45!black, anchor=west] at (3.25,2.55) {\footnotesize (a gene set $\to$ the $S_X$ subspace)};
...
signature $S_X$, a \emph{set} of genes, and the subspace onto which cross-engagement is
...
\rev{But because almost every cytokine co-induces a shared-activation program, a broadly-engaged cytokine's signature turns up in many responses and would look coupled to everything.}
...
\rev{Coupling and direction are the symmetric and antisymmetric halves of $M$; double-centring reshapes only the symmetric half, leaving the direction call unchanged.}
...
direction statistic $\mathrm{cross\_asym}=M_{ab}-M_{ba}$ untouched.}}
...
\rev{The right unit is the donor: we recompute the residual once per donor, $R_{ab}^{(d)}$ for $d=1,\dots,D$ (with $D\approx10$), and with $n_+=\#\{d:R_{ab}^{(d)}>0\}$ the number of donors in which the pair couples positively, a one-sided sign test calls it coupled when
\[
  p \;=\; 2^{-D}\sum_{k=n_+}^{D}\binom{D}{k}\;\le\;\alpha,
\]
that is, when the excess coupling is positive in more donors than a coin flip would give.}
...
Direction comes from the asymmetric part of the same matrix: $\mathrm{cross\_asym}(a,b)=M_{ab}-M_{ba}$, which is large and positive exactly when $a$'s cells carry $b$'s program but not the reverse, the upstream-to-downstream signature Claim~2 predicts \rev{(Figure~\ref{fig:asymmetry})}.
