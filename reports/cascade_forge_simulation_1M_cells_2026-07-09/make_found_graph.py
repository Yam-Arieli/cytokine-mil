...
Orientation source: pass a direction_all.csv (CascadeDirection.direction_table over the
coupled pairs) as the second argument and edges are oriented by its `cross_asym_median` --
the actual direction statistic. Without it the script falls back to the coupling table's
own `cross_asym` column, which is the M-M^T difference-of-medians *approximation*; the two
agree on every pair carrying signal but differ in sign on three isolated-negative pairs
whose |cross_asym| is ~0.001 (AMBIGUOUS, null_p >= 0.93).

Usage:
    python make_found_graph.py run_outputs/snapshot_t6.coupling.csv run_outputs/snapshot_t6.direction_all.csv
"""
...
DIRECTION_CSV = sys.argv[2] if len(sys.argv) > 2 else None
...
OUT_PANEL = Path(__file__).resolve().parent / "found_graph_panel.tex"  # bare panel for thesis.tex
...
def _direction_lookup(path):
    """{frozenset(pair): (condition_a, cross_asym_median)} from a direction_all.csv."""
    d = pd.read_csv(path)
    return {frozenset((str(r["condition_a"]), str(r["condition_b"]))):
            (str(r["condition_a"]), float(r["cross_asym_median"]))
...
def main():
    ...
    direction = _direction_lookup(DIRECTION_CSV) if DIRECTION_CSV else None
    print(f"orientation source: "
          f"{'direction_table cross_asym_median (' + DIRECTION_CSV + ')' if direction else 'coupling table cross_asym approximation'}")
    ...
    for _, r in coupled.iterrows():
        ...
        if direction is not None:
            ref_a, cm = direction[frozenset((a, b))]
            ca = cm if ref_a == a else -cm      # re-orient onto this row's (a, b)
        else:
    ...
    # Thesis-ready variant of the graph: a bare tikzpicture (the thesis wraps it in its own
    # \resizebox, as it does for every other figures/*-graph.tex) with the Stealth arrow tip
    # dropped -- thesis.tex loads tikz without arrows.meta, which is not installed there.
    OUT_PANEL.write_text(
        "% generated by make_found_graph.py -- do not edit by hand\n"
        + "\n".join(g).replace(",>={Stealth[length=2mm]}", "") + "\n"
    )
