...
Figure~\ref{fig:found} (top) shows the graph the method actually returned on one run
...
These false positives are the isolated negatives $Q,R,S,T$ spuriously coupling among
themselves and to the sink labels $I,J,K$, plus the one substantive $M\!\to\!L$ artifact
(both drive $N$, a shared-downstream common cause). The false edges are the honest cost of the
coupling gate; direction, read only on coupled pairs, is correct wherever a real cascade
exists. The lower panel of Figure~\ref{fig:found} re-plots the same 30 edges sorted by coupling
strength, which makes the separability of true cascades, transitive links, and the
isolated-negative noise floor --- and the single stubborn exception, $M\!-\!L$ --- visible at a
glance.
...
$t\!=\!6$; 30 coupled pairs). \textbf{Top:} the method's calls as a directed graph oriented by
\texttt{cross\_asym} on the Figure~\ref{fig:system} layout; \textbf{edge width $=$ coupling
strength} ($0.01$--$0.53$; see scale). \textcolor{fgood}{\textbf{Green solid}} $=$ an authored
\emph{direct} cascade recovered with correct direction (11/11); \textcolor{fgood}{\textbf{green
dashed}} $=$ a true \emph{indirect} (transitive) coupling, e.g.\ $A\!\to\!C$ via $B$ (4);
\textcolor{fbad}{\textbf{orange}} $=$ a \emph{false-positive} coupling (15); zero direct edges
missed. \textbf{Bottom:} the same 30 recovered edges, sorted by coupling strength.}
\label{fig:found}
\end{figure}
