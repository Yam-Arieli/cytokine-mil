...
$t\!=\!6$; 30 coupled pairs). Directed edges are the method's calls, oriented by
\texttt{cross\_asym}, on the Figure~\ref{fig:system} layout.
\textcolor{fgood}{\textbf{Green solid}}: an authored \emph{direct} cascade recovered with
correct direction (11/11). \textcolor{fgood}{\textbf{Green dashed}}: a true but
\emph{indirect} (transitive) coupling, e.g.\ $A\!\to\!C$ via $B$ (4).
\textcolor{fbad}{\textbf{Orange}}: a \emph{false-positive} coupling with no cascade
relationship (15). \textbf{Edge width $=$ coupling strength} (range $0.01$--$0.53$; see
scale): the real cascades are the fat arrows, the isolated-negative over-call sits at the
hairline noise floor, and the one substantive false positive, $M\!-\!L$ (a shared-downstream
artifact, both drive $N$), is the visibly thicker orange edge. Zero authored direct edges
were missed.}
\label{fig:found}
\end{figure}
