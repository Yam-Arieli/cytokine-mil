...
chance for the old score), \textbf{86\%} on mouse BMDM at 5h, and \textbf{83\%} on the
mouse in-vivo Immune Dictionary (4h, 86 cytokines; vs.\ 33\% for the old score) ---
single-frame across three datasets. The fix is exactly the intuition
...
\begin{tikzpicture}[node distance=6mm]
  ...
  \node[box, below=10mm of B, text width=5.0cm, fill=cgreen!12, draw=cgreen!60] (X)
    {\textbf{the \cross{} fix}\\ \scriptsize symmetric score $\Rightarrow$ direction-blind;\\
     \scriptsize antisymmetric \cross{} $\Rightarrow$ \textbf{88\% / 86\% / 83\%}};
...
% ================================================================== §3 data
\section{Data: cells, pseudo-tubes, and three datasets}\label{sec:data}
...
We use three datasets with opposite strengths (Table~\ref{tab:data}). Oesinghaus
...
and textbook-clean cascade biology. The Immune Dictionary (Cui 2024; mouse lymph
node, 86 cytokines, single 4h \emph{in vivo}) is a broad third test in a new
regime --- and ships expert per-cell annotations, which we pulled from the public
SCP API and joined to the GEO expression matrices by (channel, barcode).

\begin{table}[h]\centering\small
\caption{The three datasets.}\label{tab:data}
\resizebox{\textwidth}{!}{%
\begin{tabular}{@{}llll@{}}
\toprule
 & \textbf{Oesinghaus 2024} & \textbf{Sheu 2024} & \textbf{Immune Dict.\ (Cui) 2024} \\
\midrule
organism / tissue   & human PBMC                 & mouse BMDM (macrophages)        & mouse lymph node (\emph{in vivo}) \\
stimuli             & 91 cytokines (+PBS)        & 7 TLR agonists $+$ \ifnb{} (+PBS) & 86 cytokines (+PBS) \\
time points         & 24h (single)               & 0/0.25/0.5/1/3/5/8/24h (course) & 4h (single, \emph{in vivo}) \\
genes               & 4000 HVGs                  & 500-gene targeted panel         & 4000 HVGs (of $\sim$31k) \\
``donors''          & 12 biological donors       & 4 pseudo-donors @3h             & 3 mice/cytokine (rep01--03) \\
cells/tube          & $\sim$480 (16 types)       & $\sim$120 (mac\_c0--c3)         & $\sim$350 (26 expert types) \\
role here           & broad benchmark            & single-frame, multi-time        & in-vivo cross-check \\
\bottomrule
\end{tabular}}
\end{table}
...
\subsection{Immune Dictionary 4h (mouse \emph{in vivo}): 83\%, a third dataset}
The same single-frame pipeline on the Immune Dictionary --- expression from GEO,
per-cell labels from the public SCP API, expert cell types (no clustering) ---
scores \textbf{5/6 = 83\%} on a pre-registered directional benchmark (5/5 among
non-ambiguous calls; all five beat the random-gene-set null at $p\approx0$). The
symmetric \dscore{} control sits at \textbf{2/6 = 33\%} on the \emph{identical}
signatures, reproducing the symmetry argument on a third dataset. The method
recovers the Dictionary's own canonical NK cascade
(IL-2/IL-15/IL-18$\to$IFN-$\gamma$, all correct) and TNF$\to$IL-6 (\nfkb$\to$STAT3).
The single miss, IL-1$\beta\to$IL-6, is a near-zero \emph{non}-call that fails the
null (not a wrong direction) --- the same signature-collapse mode as polyIC, while
the \emph{parallel} TNF$\to$IL-6 cascade recovers cleanly.
...
only directs those.
\item \textbf{Small benchmarks.} 17, 7, and 6 graded cascades. Each dataset's one
failure is a single explained signature-collapse (polyIC on Sheu; IL-1$\beta$ on
the Immune Dictionary), not noise.
...
The method works at $\sim$83--88\% direction accuracy on \emph{three} independent
datasets (human ex-vivo PBMC, mouse ex-vivo BMDM, mouse in-vivo lymph node),
...
31--41 & results: Oes 88\%, Sheu kinetics, ID 83\% (3rd dataset), polyIC wart & \textbf{F9}, F11, \textbf{F12} \\
...
Immune Dict.\ Stage 1+2 (Path A) & 128 & 512 & 256 & 64  & 87 \\
Immune Dict.\ binary (Bridge)    & 512 & 512 & 512 & 128 & 2 \\
\bottomrule
\end{tabular}\\[2pt]
...
Immune Dict.\ Stage 1+2  & 50 & 0.01  & 20  & 0.001 & 5 \\
Immune Dict.\ binary     & 20 & 0.005 & 250 & 3e-5  & --- \\
\bottomrule
\end{tabular}\\[2pt]
...
M2\_IL4\_rep1; Oesinghaus val donors 2,3; Immune Dict.\ val rep03.}
\end{table}
