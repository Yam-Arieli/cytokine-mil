...
- **Eval (M8):** sign(median cross_asym) vs `expected_sign` → **88% Oes / 86% Sheu-5h**;
  `directional_score` (symmetric) = 47% on the same data.

One-line thesis: **`directional_score` is symmetric (direction-blind); the antisymmetric
`cross_asym` recovers cascade direction from a single snapshot — 88%/86% on two datasets.**
...
The `cross_asym` direction method works single-frame, no-leakage, on two datasets (88% / 86%).
