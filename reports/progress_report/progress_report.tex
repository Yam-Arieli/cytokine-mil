...
condition $b$'s signature, control-normalised, computed \emph{per cell type} $T$:
\begin{equation}
s_T(a,S_b)\;=\;\bar\sigma_{S_b}\big(a\text{'s }T\text{-cells}\big)\;-\;\bar\sigma_{S_b}\big(\text{control's }T\text{-cells}\big),
\qquad \bar\sigma_S(\cdot)=\text{mean expression of gene set }S .
\end{equation}
Aggregating each ordered pair by the median across cell types gives the cross-engagement matrix
$M[a,b]=\operatorname{median}_T s_T(a,S_b)$, whose \emph{symmetric} half answers existence:
\begin{equation}
\underbrace{\text{coupling}(a,b)=M[a,b]+M[b,a]}_{\text{symmetric}\;\Rightarrow\;\text{do they engage each other? (existence)}}
\end{equation}
reported as its degree-corrected (double-centred) residual --- see Section~\ref{sec:gate-fix}.
Direction is the \emph{antisymmetric} counterpart, but it is aggregated differently: the
asymmetry is formed \emph{within} each cell type and only then medianed, over the cell types in
which \emph{both} conditions (and the control) are measured:
\begin{equation}
\mathrm{cross\_asym}(a,b)\;=\;\operatorname*{median}_{T}\;\underbrace{\big[s_T(a,S_b)-s_T(b,S_a)\big]}_{\text{per-cell-type cross-engagement asymmetry}} .
\label{eq:cross}
\end{equation}
(A median of differences, not a difference of medians: the median is not linear, and $M[a,b]$
and $M[b,a]$ are each aggregated over their own cell-type set, so $M[a,b]-M[b,a]$ is a related
but distinct quantity.)
...
\section{The coupling-gate fix}\label{sec:gate-fix}
